...
## Feature selection in non-lenear models 
Using default regsubsets from leaps library so only max 8 features are sselected.

This part seems sensless but whatever


```{r}
library(leaps)
fit.bs <- regsubsets(alc ~ .,data = data,nvmax = 10)
summary(fit.bs)
fit.bs.summary$cp
...
As we can see in the above analysis the best choice is to take 6 features namely:
sexM, addressU, studytime, paidyes, famrel, goout.
...
fit.step <- lm(alc ~ cut(absences, 6), data = data)
...
plot(absences, alc, col = "darkgrey", cex = 0.5, xlim = absences.lims)
...
fit.step <- lm(alc ~ cut(age, 6), data = data)
...
After pruning optimal tree has only 2 levels and most important nodes are: gout and G3(final grade). estimated error rate is equal to around 26%
...
alc.bag <- randomForest(alc ~ ., data = data, mtry = 3, importance = TRUE)
alc.bag
...
attach(data)
set.seed(1)
alc.bag <- randomForest(alc ~ ., data = data, subset = train, mtry = 3,
                         importance = TRUE,ntree = 250)
alc.pred.bag <- predict(alc.bag, newdata = data[test,])
mean((alc.pred.bag - data$alc[test])^2)
...
attach(data)
set.seed(2)
alc.bag.s <- randomForest(alc ~ ., data = data, subset = train, mtry = 3,
                         importance = TRUE, ntree = 10)
alc.pred.bag <- predict(alc.bag.s, newdata = data[test,])
mean((alc.pred.bag - data$alc[test])^2)
